...
function customInputFile(wrapper){
    ...
    $(inputFile).each(function(){
        ...
        $(this).wrap("<a href='#' class='custom-file'></a>");
        $(this).parent().prepend('<span class="custom-file-result-text">No file chosen</span>');
        $(this).parent().prepend('<span class="custom-file-trigger-text">Choose file</span>');
        $(this).fadeTo(0,0.0);
        /*if($(this).is(':checked')){
            $(this).parent().addClass("selected");
            $(this).parents("label").addClass("selected");
...
function init(wrapper){
    ...
    $(document).find(".slide ul").bxSlider({
        mode:"horizontal",
        controls:true,
        pager:false,
        prevText:"",
    ...
    $(document).find(".carousel ul").bxSlider({
        mode:"horizontal",
        slideWidth: 260,
        slideMargin: 20,
        minSlides: 4,
    ...
    $(document).find(".instagram-photos ul").bxSlider({
        mode:"horizontal",
        slideWidth: 260,
        slideMargin: 20,
        minSlides: 4,
    ...
    $(document).find(".gallery-pager ul").bxSlider({
        mode:"vertical",
        minSlides: 4,
        maxSlides: 4,
        moveSlides: 1,
    ...
    wrapper.find(".tabs-collapsible").tabs({
        collapsible :true
    });
    wrapper.find(".tabs-no-active").tabs({
        active:false,
        collapsible :true
    });
    ...
    wrapper.find("#price-range").slider({
        range: true,
        min: 0,
        max: 300,
        values: [40,200],
        ...
        slide: function( event, ui ) {
            ...
            wrapper.find("#price-range .value.value-min i").text(ui.values[0]);
            wrapper.find("#price-range .value.value-max i").text(ui.values[1]);

            //console.log(ui.values[0],ui.values[1])
        }
    });
    wrapper.find("#price-range .value.value-min i").text(wrapper.find("#price-range").slider("values",0));
    wrapper.find("#price-range .value.value-max i").text(wrapper.find("#price-range").slider("values",1));
...
function showModalbox(url){
    ...
    $.ajax({
        ...
        beforeSend: function(data) {
            ...
            $(document).find("html").addClass("modalbox-open");
        ...
        success: function(data) {
            ...
            var innerHtml = '<div class="modalbox">'+data+'</div>';
            $(document).find("body").append(innerHtml);

            var modalbox = $(document).find(".modalbox").eq(-1);

            modalbox.fadeIn(400,function(){
                init(modalbox);
            });
...
function changeModalbox(url){
    ...
    $.ajax({
        ...
        beforeSend: function(data) {
            ...
            $(document).find("html").addClass("modalbox-open");
            $(document).find(".modalbox").eq(-1).empty();
        ...
        success: function(data) {
            ...
            //var innerHtml = '<div class="modalbox">'+data+'</div>';
            //$(document).find("body").prepend(innerHtml);
...
function removeModalbox(){
    ...
    $(document).find("html").removeClass("modalbox-open");
    $(document).find(".modalbox").eq(-1).fadeOut(400,function(){
        $(this).remove();
    });
...
function showFilter(button){
    ...
    if(checkboxes.length > 0)
    {
        ...
        $.each(checkboxes,function(i,checkbox){
            ...
            var text = checkbox.parents("label").find(".label-text").text();
            string += text+", "

        });
        var innerHtml = '<div class="products-filters">' +
            '<div class="products-filters-title">Filtre tercihleri: <span>'+string.slice(0,-2)+'</span></div>' +
            '<a href="#" onclick="javascript:clearFilters();">Temizle</a>' +
            '</div>';

        console.log(innerHtml);
        $(document).find(".products").before(innerHtml);
        //innerHtml.before($(document).find(".products"));
...
function clearFilters(){
    ...
    $(document).find(".products-filters").slideUp(400,function(){
        $(this).remove();
    });
...
function addMarker(map, store, lat, lng) {
    var myLatlng = new google.maps.LatLng(lat,lng);
    var image = new google.maps.MarkerImage("assets/images/icon-map-pin.png",
